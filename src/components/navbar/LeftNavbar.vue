<template>
  ...
  >
    ...
    >
      <a
        class="mb-xl-3 mb-md-2 nav-link"
        data-toggle="pill"
        href="#nav-tab-user"
        @click="jump(0)"
        role="tab"
      >
        <img
          src="http://v.bootstrapmb.com/2021/2/yxh1x9666/assets/images/user.png"
      ...
      ></a>
      <a
        class="mt-xl-3 mt-md-2 nav-link light-dark-toggle"
      ...
      <router-link
        ...
        to="/welcome/Settings"
        role="tab"
        ><i class="zmdi zmdi-settings"  @click="closeBar()"></i
      ></router-link>
    </div>

    <button type="submit" class="btn sidebar-toggle-btn shadow-sm" @click="changeBarView()">
...
export default {
  ...
  methods: {
    jump(m) {
      document.getElementsByName("nav-tab").forEach(e=>{
        e.classList.remove("show")
        e.classList.remove("active")
      })
      var view = [true, true, true, true, true];
      view[m] = false;
      console.log(m);
      this.$emit("updateView", view);
      var body = document.getElementsByTagName("html")[0]
      body.classList.add("open-sidebar-menu")
    },
    changeTheme() {
    ...
    changeBarView(){
      var body = document.getElementsByTagName("html")[0]
      if(body.classList.contains("open-sidebar-menu")){
